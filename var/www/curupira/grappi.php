...
$pagina = new Pagina();
$pagina->comeca("Gr&aacute;fico de Impress&otilde;es", $_SERVER["PHP_SELF"]."?".$_SERVER["QUERY_STRING"],$_POST, $_GET);
...
if(isset($retval))
  ...
  $retval = '<fieldset>
<legend accesskey="g">&nbsp;<font size="2"><b>Gr&aacute;fico de Impress&otilde;es P&aacute;ginas por Impressoras</b></font>&nbsp;</legend>
<form name="frm_data" method="post" action="'.$_SERVER["PHP_SELF"].'">
<table border="0" width="100%" cellpadding="0" cellspacing="0">';
$dataI = (isset($_POST["dtinicial"]))?$_POST["dtinicial"]:"01/".date("m/Y
",time());
...
if($_SESSION["perfil"]==2 || $_SESSION["perfil"]==3){
	...
	$retval .='<select name="codunidade" style="font-size:8pt"><option value="-1">:: Todas Unidades ::</option>';
...
if($salvar==0){
  ...
  if(isset($_POST["dtinicial"]) && isset($_POST["dtfinal"]) )
    echo '<center><img src="pizzappi.php?datai='.$_POST["dtinicial"].'&dataf='.$_POST["dtfinal"].'&codunidade='.((isset($_POST["codunidade"]))?$_POST["codunidade"]:-1).'" /></center>';
  else
		echo '<center><img src="pizzappi.php?codunidade='.((isset($_POST["codunidade"]))?$_POST["codunidade"]:-1).'" /></center>';
	...
	header("location: pizzappi.php"."?datai=".$_POST["dtinicial"]."&dataf=".$_POST["dtfinal"]."&codunidade=".((isset($_POST["codunidade"]))?$_POST["codunidade"]:-1)."&salvar=".$salvar);
}
?>
